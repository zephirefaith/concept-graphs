...
def get_parser():
    parser = argparse.ArgumentParser(
        description="Visualize a series of point clouds as an animation."
    )
    parser.add_argument(
        "--input_folder",
        type=str,
...
def main(args):
    ...
    video_save_path = (
        Path(args.input_folder).parent / f"{cfg.gsa_variant}_{cfg.save_suffix}.mp4"
    )

    # Save each video separately in this folder
    sep_video_save_folder = (
        Path(args.input_folder).parent.parent
        / "anime_all_frames"
    ...
    if "npy" in color_path:
        color_path = color_path.replace("npy", "jpg")
    ...
    for i in trange(len(frame_paths)):
        ...
        frame_classid_to_svcolor = [
            class_name_to_sv_color[class_name] for class_name in frame_class_names
        ]
