...
def main(args: argparse.Namespace):
    ...
    for idx in trange(len(dataset)):
        ...
        vis_save_path = str(vis_save_path).replace("npy", "jpg")
        detections_save_path = str(detections_save_path).replace("npy", "jpg")

        if "jpg" in color_path:
            image = cv2.imread(color_path)  # This will in BGR color space
            # Convert to RGB color space
            image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        elif "npy" in color_path:
            image_rgb = np.load(color_path)
            image = cv2.imread(color_path.replace("npy", "jpg"))
